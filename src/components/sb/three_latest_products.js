...
export default async function Three_latest_products({ blok }) {
    const products = await CMS.getThreeProducts();
    console.log(products)
    const latestProducts = products?.sort((a, b) => new Date(b.content.created_at) - new Date(a.content.created_at)).filter(a => a.name !== "products")
